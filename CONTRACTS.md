...
Camovar uses deterministic local lake artifacts under a `LakePaths` root. Table paths ending in `.parquet` are physical Apache Parquet files written through `camovar.table_io`; JSON and CSV artifacts keep their native formats.
...
```bash
uv run camovar search "UCITS ETF"
uv run camovar metadata-filter --name-contains "UCITS ETF" --selection-name ucits-etf
uv run camovar univariate-statistics --root lake
uv run camovar bivariate-statistics --root lake
```
