...
`camovar.risk_model` is a pure-computation module: it accepts aligned Gold
...
lake and does not depend on `camovar.evaluation` or `camovar.portfolio`, so
...
- `sample`: the classic unbiased sample covariance (denominator `T - 1`),
  numerically consistent with `camovar.gold_pair_stats.sample_covariance`.
