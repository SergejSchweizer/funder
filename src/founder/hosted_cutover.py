...
from founder.hosted_readiness import public_hosted_mode_allowed
...
def run_hosted_cutover_proof() -> HostedCutoverReport:
    ...
    vault = EodhdCredentialVault(
        ...
        fingerprint_secret=b"founder-cutover-fingerprint-secret",
    )
...
def _local_cli_contract_present() -> bool:
    cli_source = (REPOSITORY_ROOT / "src" / "founder" / "cli.py").read_text(encoding="utf-8")
    return all(
