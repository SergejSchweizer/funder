...
`founder.portfolio_parts.solvers` (projected gradient descent) and
`founder.portfolio_parts.clustering` (hierarchical clustering).
...
dynamically re-imported from the `founder.portfolio` facade.
"""
...
from founder.portfolio_parts.solvers import project_capped_simplex
...
def historical_var_and_cvar(
    ...
    `founder.evaluation`'s historical tail-risk metrics).
    """
