...
`founder.portfolio` facade. The repository has no numerical runtime
dependency (pyarrow only); this module hand-implements clustering rather than
depending on scipy, matching the precedent set by `founder.risk_model`'s
Jacobi eigenvalue solver and `founder.portfolio_parts.solvers`'s projected
gradient descent.
"""
