...
class RefreshService:
    ...
    def __init__(
        ...
        *,
        ...
        self._current_pointer: CurrentRefreshPointer | None = None
    ...
    def synchronize_catalog(self, request: RefreshRequest) -> RefreshResult:
        ...
        if request.publish:
            empty_market_set = MarketDataVersionSet(())
            self._current_pointer = CurrentRefreshPointer(
                refresh_snapshot_id=snapshot.catalog_snapshot_id,
                catalog_snapshot_id=snapshot.catalog_snapshot_id,
                market_data_version_set_id=empty_market_set.market_data_version_set_id,
    ...
    def build_market_data_version_set(
        self,
        *,
        dataset: str,
        rows_by_listing: dict[str, tuple[JsonRow, ...]],
    ) -> MarketDataVersionSet:
        versions: list[MarketDatasetVersion] = []
        for listing_id, rows in sorted(rows_by_listing.items()):
            dates = sorted(str(row["date"]) for row in rows if "date" in row)
            versions.append(
                MarketDatasetVersion(
                    dataset=dataset,
                    listing_id=listing_id,
                    content_fingerprint=stable_row_fingerprint(rows),
                    first_date=dates[0] if dates else "",
                    last_date=dates[-1] if dates else "",
                )
            )
        return MarketDataVersionSet(tuple(versions))
    ...
    def publish_current_pointer(
        ...
        *,
        ...
        snapshot: CatalogSnapshot,
        market_data_version_set: MarketDataVersionSet,
    ) -> CurrentRefreshPointer:
        if not snapshot.completeness.is_complete:
            raise ValueError("cannot publish incomplete refresh snapshot")
        pointer = CurrentRefreshPointer(
            refresh_snapshot_id=snapshot.catalog_snapshot_id,
            catalog_snapshot_id=snapshot.catalog_snapshot_id,
            market_data_version_set_id=market_data_version_set.market_data_version_set_id,
            run_id=request.run_id,
        )
        self._current_pointer = pointer
        return pointer

    def current_pointer(self) -> CurrentRefreshPointer | None:
        return self._current_pointer

    def result_summary(self, result: RefreshResult) -> JsonRow:
        snapshot = result.snapshot
        return {
            "error_count": len(result.errors),
            "exchange_count": result.request.exchanges,
            "listing_count": result.listing_count,
            "missing_isin_count": len(result.missing_isins),
            "run_id": result.request.run_id,
            "status": result.status.value,
            "unique_isin_count": result.unique_isin_count,
            "historical_nav_capable_count": 0
            if snapshot is None
            else sum(1 for listing in snapshot.listings if listing.historical_nav_available),
        }
...
def stable_row_fingerprint(rows: tuple[JsonRow, ...]) -> str:
    from founder.contract_versioning import stable_contract_id

    return stable_contract_id("rows", tuple(sorted(tuple(sorted(row.items())) for row in rows)))
