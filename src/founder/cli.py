...
def build_parser() -> argparse.ArgumentParser:
    ...
    bronze = subparsers.add_parser("bronze", help="Run Bronze planning for the approved universe.")
    bronze.add_argument(
        "--debug",
        action="store_true",
        default=argparse.SUPPRESS,
    ...
    bronze.add_argument("--end-date", type=_parse_date, help="Optional bronze end date YYYY-MM-DD.")
    bronze.add_argument("--run-date", type=_parse_date, help="Archive run date YYYY-MM-DD.")
    bronze.add_argument(
        "--concurrency",
        type=_positive_int,
        default=2,
    ...
    bronze_selector = bronze.add_mutually_exclusive_group()
    bronze_selector.add_argument(
        "--limit",
        type=_positive_int,
    ...
    silver = subparsers.add_parser("silver", help="Build Silver quote files from Bronze quotes.")
    silver.add_argument(
        "--debug",
        action="store_true",
        default=argparse.SUPPRESS,
...
def main(argv: Sequence[str] | None = None) -> None:
    ...
    if args.command == "bronze":
        summary = _run_bronze_command(args)
        print(json.dumps(summary, sort_keys=True))
        return
...
def _run_bronze_command(args: argparse.Namespace) -> dict[str, Any]:
    paths = LakePaths(root=Path(args.root))
    run_date = args.run_date or date.today()
    ...
    run_id = args.run_id or _generated_run_id("bronze", run_date=run_date)
    canonical_path = resolve_current_universe(paths)
    LOGGER.info("running bronze run_id=%s canonical_path=%s", run_id, canonical_path)
    with bronze_run_lock(paths, run_id):
        ...
        if gap_aware:
            ...
            write_rows(paths.bronze_plan(run_id), quote_plan)
        summary: dict[str, Any] = {
    ...
    LOGGER.info("bronze complete run_id=%s plan_rows=%s", run_id, len(quote_plan))
    return summary
