...
The hosted API is exposed by `founder.hosted_api` and mounted in the API container. It provides user-scoped session, credential, download, dataset, project, metadata-filter project creation, selection, analysis, metrics, returns, weights, report, and account-deletion routes for the Web UI. The API container mounts `./lake` read-only at `/srv/founder/lake` so `GET /metadata-filter/options` can populate project-definition dropdowns from `lake/reference/all_isins/all_isins.parquet`. If the host lake is group-restricted, set `FOUNDER_LAKE_GROUP_ID` in `.env.local` to the host group id that can read the lake; the default is `10`.

The hosted Web container serves the local research workspace from `apps/web/server.js`. It provides the production shell baseline with Google-style Material color tokens, restrained elevation, a project-scoped sidebar, and a Project Snapshot selector. The topbar contains the write-only EODHD key input and `Fetch all ISINs` action. Project Snapshot, `Projects`, and Project Definition stay disabled while the EODHD key field is empty or until `Fetch all ISINs` succeeds. The sidebar starts with only `Projects`; project names appear hierarchically below it after metadata-filter projects exist. With no selected project, the workspace shows the Project Definition form for ISIN search filters: exchange, name substring, instrument type, country, and currency. The `Create New Project` button calls the metadata-filter project endpoint, creates a project named from structured filter values such as exchange, instrument type, country, and currency, creates the resulting selection, refreshes the project list, and selects the new project. Free-text name searches filter membership but are not embedded in project names. The persisted funnel routes `Data -> Metadata -> Univariate -> Filter -> Diversification -> Portfolio -> Validation -> Report` are hidden until a project is selected. The dashboard shell is shown only after the Web surface has an authenticated session. `FOUNDER_AUTH_MODE=google` is the default and requires real Google OIDC; login fails closed when Google OAuth config is incomplete. Set `FOUNDER_AUTH_MODE=local-dev` only for offline Docker development, where `/auth/google/start` creates a visibly labelled `local-dev-google` session instead of contacting Google.

Real Google OIDC requires a Google OAuth client configured with the exact redirect URI in `FOUNDER_GOOGLE_REDIRECT_URI`, normally `http://localhost:3000/auth/google/callback` for local development or the HTTPS hosted callback in deployment. Set `FOUNDER_GOOGLE_CLIENT_ID` in `.env.local`; the client secret must live in the external file referenced by `FOUNDER_GOOGLE_CLIENT_SECRET_FILE`; it is mounted into the Web container as a Docker secret and must never be committed. The Web runtime creates PKCE, state, and nonce values, redirects to Google's account chooser, exchanges the callback code server-side, verifies the Google ID token using JWKS, and renders the lowercase Google email or display name under `Founder Research`.
