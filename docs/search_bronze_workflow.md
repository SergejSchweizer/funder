...
The production-style `vcs` crontab runs Refresh daily at 18:00 local server time. The job keeps absolute paths in variables because cron has a minimal environment, and wraps Refresh with `flock -n` so overlapping runs are skipped instead of writing into the lake concurrently:
...
Use `crontab -l` as user `vcs` to inspect the installed schedule. Logs are appended to `.logs/cron-refresh.log`.
