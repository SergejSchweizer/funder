...
This guide explains how to use the Search and Fetch modules directly from Python. It is written for someone who needs to produce a reviewed ETF universe, ingest quote inputs into Fetch, and inspect coverage without first learning the whole codebase.
...
EODHD search or exchange-symbol-list rows
    ...
    +-------+-------+
        |
        | Fetch rows + operational metadata
        v
    +-------+-------+
    | gold and      |
    | portfolio     |
...
- Fetch plans symbols, archives raw or near-raw Fetch data, and writes coverage manifests.

Search must not ingest historical quote payloads. Fetch must not perform fuzzy discovery. The handoff is the `canonical_universe` table and the `current_universe.json` pointer.
...
`founder.fetch` is the module to use after a universe has been approved. It reads the canonical-universe contract, validates required fields, derives EODHD symbols, writes a fetch plan, archives quote, dividends, and splits data into Fetch, writes coverage manifests, and logs Fetch errors to the run log.
...
`founder.schemas` is the module to use when code needs to validate table shape. It defines the required fields that Search and Fetch must preserve when moving rows between Fetch, Silver, Gold, metadata, and coverage.
...
Fetch is safe for unattended cron execution. Live EODHD loading uses bounded parallelism with default concurrency `2`, while still honoring shared request pacing, retry backoff, and `Retry-After`. Cron execution should use stable run ids, be resumable after partial failures, and prevent or clearly report overlapping runs for the same lake root and run id.

`--mock` writes deterministic local Fetch quote and operational metadata outputs without using an EODHD token:

```bash
uv run founder fetch --mock
```

Without `--mock`, `founder fetch` writes the fetch plan, archives live Fetch quote, dividend, and split rows through the same planned windows, and writes coverage/fetch-run metadata. With `--mock`, it writes deterministic local Fetch quote, coverage, and fetch-run metadata. Optional flags such as `--root`, `--run-id`, `--start-date`, `--end-date`, `--run-date`, `--concurrency`, `--limit`, and `--isin` are available for reproducible custom runs, but they are not required.
...
After an initial full-history fetch, later live loads inspect already stored Silver quotes before downloading. The planner coalesces missing quote dates into one backfill window per ISIN, then loads through the selected run date so repeated small historical holes do not create hundreds of API calls:
...
Use Python when you need custom loaders or want to embed Search and Fetch in another workflow. This copy-paste command uses mocked quote payloads, so it does not need an EODHD token:
...
coverage = write_fetch_manifests(paths, run_id=bronze_run_id, quote_rows=quotes)
...
Use `founder.search` when you have raw EODHD search or exchange symbol-list rows and need a fetch-ready canonical universe.
...
- Search raw payloads under `lake/bronze/eodhd/search/run_date=YYYY-MM-DD/`.
...
### Fetch Quotes To Fetch

```python
from founder.config import load_eodhd_config
from founder.fetch import eodhd_quote_loader, write_quotes_to_bronze
...
| --- | --- | --- |
| Search candidates | `write_search_run` | Fetch search payload, Silver candidates |
...
| Quote archive | `write_quotes_to_bronze` | Fetch quote payloads, error rows |
...
| Additional EODHD data | `write_raw_eodhd_datasets_to_bronze` | Fetch dividends and splits rows using the same planned windows as quotes |
| Coverage | `write_fetch_manifests` | Coverage rows, fetch-run manifest, coverage CSV |

## Common Failure Modes

- Missing ISINs are not fetch-ready. Review Search summaries before approval.
